...
describe("helper functions", () => {
  ...
  describe("getUri", () => {
    ...
    describe("without explicit ports", () => {
    ...
    describe("with explicit ports", () => {
      it("has X-Forwarded headers", () => {
        ...
        expect(getUri(event)).toEqual("https://forward-host:8443/path");
      });
      it("does not have X-Forwarded headers", () => {
        const event = mockEvent({
          headers: {
        ...
        expect(getUri(event)).toEqual("http://host:8080/path");
      });
      it("respects forceHost setting", () => {
        process.env.forceHost = "forced-host:3000";
        const event = mockEvent({
          headers: {
            "x-forwarded-proto": "https",
            "x-forwarded-host": "forward-host",
        ...
        expect(getUri(event)).toEqual("https://forced-host:3000/path");
      });
    });
  });
