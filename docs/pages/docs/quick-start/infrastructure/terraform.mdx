...
export async function getStaticProps(context) {
  ...
  const outputs = [
    ...
    { Name: 'serverless_iiif_metadata_function_arn', Description: 'Metadata Function ARN' }
  ]
...
export const Input = ({ AllowedPattern, AllowedValues, Default, Description, MaxLength, MaxValue, MinLength, MinValue, Name, Type }) => {
  const DefaultString = Default?.toString();
  return <>
    <div className={styles.tfAttribute}>
      ...
      <Attribute Name="Default" Value={DefaultString} Code={true}/>
    </div>
  </>
}
